...
const marchingCubeInstance = new MarchinCube(configuration.chunkSize, configuration.chunkLimit, on_sample_callback);
...
const onMainScriptMessage = (event: MessageEvent) => {
    ...
    const on_vertex_callback = (vertex: Vec3, color: Vec3, normal: Vec3) => {
        ...
        if (bufIndex + 12 > configuration.workerBufferSize)
            return;
